package com.luxegem.invoice.auth.controller;

import com.luxegem.invoice.auth.entity.AppUser;
import com.luxegem.invoice.auth.model.AuthResponse;
import com.luxegem.invoice.auth.model.LoginRequest;
import com.luxegem.invoice.auth.model.RegisterRequest;
import com.luxegem.invoice.auth.model.UpdateProfileRequest;
import com.luxegem.invoice.auth.model.ChangePasswordRequest;
import com.luxegem.invoice.auth.model.ForgotPasswordRequest;
import com.luxegem.invoice.auth.model.ResetPasswordRequest;
import com.luxegem.invoice.auth.model.VerifyOtpRequest;
import com.luxegem.invoice.auth.entity.PasswordResetToken;
import com.luxegem.invoice.auth.model.UserDetailsResponse;
import com.luxegem.invoice.auth.repository.AppUserRepository;
import com.luxegem.invoice.auth.repository.PasswordResetTokenRepository;
import com.luxegem.invoice.auth.service.JwtTokenService;
import com.luxegem.invoice.auth.service.EmailService;
...
public class AuthController {
    ...
    @PutMapping("/profile")
    @Transactional
    public Map<String, String> updateProfile(@RequestBody UpdateProfileRequest request, Principal principal) {
        String shopId = principal.getName();
        AppUser user = appUserRepository.findByShopId(shopId)
                .or(() -> appUserRepository.findByUsername(shopId))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found"));
        ...
        return Map.of(
                ...
                "gstNumber", user.getGstNumber() != null ? user.getGstNumber() : "");
    }

    @PostMapping("/change-password")
    @Transactional
    public Map<String, String> changePassword(@Valid @RequestBody ChangePasswordRequest request, Principal principal) {
        String shopId = principal.getName();
        AppUser user = appUserRepository.findByShopId(shopId)
                .or(() -> appUserRepository.findByUsername(shopId))
                .orElseThrow(() -> new ResponseStatusException(HttpStatus.NOT_FOUND, "User not found"));
